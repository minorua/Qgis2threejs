...
var w={debug:{},workerPath:function(){if(document.currentScript&&document.currentScript.src){var a=(new URL(document.currentScript.src+'/..')).href;'/'===a.slice(-1)&&(a=a.slice(0,-1));return a}console.warn('Potree: Was unable to find its script path using document.currentScript.');return''}(),maxNodesLoadGPUFrame:20,maxDEMLevel:0,maxNodesLoading:void 0!==navigator.hardwareConcurrency?navigator.hardwareConcurrency:4,pointLoadLimit:1E10,numNodesLoading:0,measureTimings:!1,workerPool:new A,lru:new na,
pointcloudTransformVersion:void 0},u={POSITION_CARTESIAN:0,COLOR_PACKED:1,COLOR_FLOATS_1:2,COLOR_FLOATS_255:3,NORMAL_FLOATS:4,FILLER:5,INTENSITY:6,CLASSIFICATION:7,NORMAL_SPHEREMAPPED:8,NORMAL_OCT16:9,NORMAL:10,RETURN_NUMBER:11,NUMBER_OF_RETURNS:12,SOURCE_ID:13,INDICES:14,SPACING:15},C={DATA_TYPE_DOUBLE:{ordinal:0,size:8},DATA_TYPE_FLOAT:{ordinal:1,size:4},DATA_TYPE_INT8:{ordinal:2,size:1},DATA_TYPE_UINT8:{ordinal:3,size:1},DATA_TYPE_INT16:{ordinal:4,size:2},DATA_TYPE_UINT16:{ordinal:5,size:2},DATA_TYPE_INT32:{ordinal:6,
size:4},DATA_TYPE_UINT32:{ordinal:7,size:4},DATA_TYPE_INT64:{ordinal:8,size:8},DATA_TYPE_UINT64:{ordinal:9,size:8}},oa=0,pa;for(pa in C)C[oa]=C[pa],oa++;g.POSITION_CARTESIAN=new g(u.POSITION_CARTESIAN,C.DATA_TYPE_FLOAT,3);g.RGBA_PACKED=new g(u.COLOR_PACKED,C.DATA_TYPE_INT8,4);g.COLOR_PACKED=g.RGBA_PACKED;g.RGB_PACKED=new g(u.COLOR_PACKED,C.DATA_TYPE_INT8,3);g.NORMAL_FLOATS=new g(u.NORMAL_FLOATS,C.DATA_TYPE_FLOAT,3);g.FILLER_1B=new g(u.FILLER,C.DATA_TYPE_UINT8,1);g.INTENSITY=new g(u.INTENSITY,C.DATA_TYPE_UINT16,
1);g.CLASSIFICATION=new g(u.CLASSIFICATION,C.DATA_TYPE_UINT8,1);g.NORMAL_SPHEREMAPPED=new g(u.NORMAL_SPHEREMAPPED,C.DATA_TYPE_UINT8,2);g.NORMAL_OCT16=new g(u.NORMAL_OCT16,C.DATA_TYPE_UINT8,2);g.NORMAL=new g(u.NORMAL,C.DATA_TYPE_FLOAT,3);g.RETURN_NUMBER=new g(u.RETURN_NUMBER,C.DATA_TYPE_UINT8,1);g.NUMBER_OF_RETURNS=new g(u.NUMBER_OF_RETURNS,C.DATA_TYPE_UINT8,1);g.SOURCE_ID=new g(u.SOURCE_ID,C.DATA_TYPE_UINT8,1);g.INDICES=new g(u.INDICES,C.DATA_TYPE_UINT32,1);g.SPACING=new g(u.SPACING,C.DATA_TYPE_FLOAT,
1);t.prototype.add=function(a){this.attributes.push(a);this.byteSize+=a.byteSize;this.size++};t.prototype.hasColors=function(){for(var a in this.attributes)if(this.attributes[a].name===u.COLOR_PACKED)return!0;return!1};t.prototype.hasNormals=function(){for(var a in this.attributes){var b=this.attributes[a];if(b===g.NORMAL_SPHEREMAPPED||b===g.NORMAL_FLOATS||b===g.NORMAL||b===g.NORMAL_OCT16)return!0}return!1};class ea{constructor(a,b,c){this.name=a;this.box=b;this.tileSize=c;this.level=this.name.length-
1;this.data=new Float32Array(c*c);this.data.fill(-Infinity);this.children=[];this.mipMap=[this.data];this.mipMapNeedsUpdate=!0}createMipMap(){this.mipMap=[this.data];for(var a=this.tileSize,b=parseInt(a/2),c=this.data;1<b;){for(var f=new Float32Array(b*b),d=0;d<b;d++)for(var l=0;l<b;l++){var k=c[2*d+2*l*a],v=c[2*d+2*l*a+a],h=c[2*d+1+2*l*a],m=c[2*d+1+2*l*a+a],[n,e]=[0,0];isFinite(k)&&(n+=k,e+=1);isFinite(v)&&(n+=v,e+=1);isFinite(h)&&(n+=h,e+=1);isFinite(m)&&(n+=m,e+=1);n/=e;f[d+l*b]=n}this.mipMap.push(f);
c=f;a=b;b=parseInt(b/2)}this.mipMapNeedsUpdate=!1}uv(a){var b=this.box.getSize(new THREE.Vector3);return[(a.x-this.box.min.x)/b.x,(a.y-this.box.min.y)/b.y]}heightAtMipMapLevel(a,b){a=this.uv(a);var c=parseInt(this.tileSize/parseInt(Math.pow(2,b))),f=this.mipMap[b];b=Math.min(a[0]*c,c-1);a=Math.min(a[1]*c,c-1);var d=b%1,l=a%1,[k,v]=[Math.floor(b),Math.ceil(b)],[h,m]=[Math.floor(a),Math.ceil(a)];b=f[k+c*h];a=f[k+c*m];var n=f[v+c*h];c=f[v+c*m];f=isFinite(b)?(1-d)*(1-l):0;var e=isFinite(a)?(1-d)*l:0,
p=isFinite(n)?d*(1-l):0;d=isFinite(c)?d*l:0;l=f+e+p+d;f/=l;e/=l;p/=l;d/=l;if(0===l)return null;l=0;isFinite(b)&&(l+=b*f);isFinite(a)&&(l+=a*e);isFinite(n)&&(l+=n*p);isFinite(c)&&(l+=c*d);return l}height(a){for(var b=null,c=0;c<this.mipMap.length&&(b=this.heightAtMipMapLevel(a,c),null===b);c++);return b}traverse(a,b=0){a(this,b);for(var c of this.children.filter(a=>void 0!==a))c.traverse(a,b+1)}}class qa{constructor(a){this.pointcloud=a;this.boundingBox=this.matrix=null;this.tileSize=64;this.root=
null;this.version=0}expandAndFindByBox(a,b){if(0===b)return[this.root];for(var c=[],f=[this.root];0<f.length;){var d=f.pop(),l=d.box.getSize(new THREE.Vector3),k=(a.min.x-d.box.min.x)/l.x,v=(a.min.y-d.box.min.y)/l.y,h=(a.max.x-d.box.max.x)/l.x,m=(a.max.y-d.box.max.y)/l.y;k=.5>k?0:1;v=.5>v?0:1;h=.5>h?0:1;m=.5>m?0:1;k=0===k&&0===v&&1===h&&1===m?[0,1,2,3]:k===h&&v===m?[k<<1|v]:[k<<1|v,h<<1|m];for(var n of k)void 0===d.children[n]&&(k=d.box.clone(),0<(n&2)?k.min.x+=l.x/2:k.max.x-=l.x/2,0<(n&1)?k.min.y+=
l.y/2:k.max.y-=l.y/2,k=new ea(d.name+n,k,this.tileSize),d.children[n]=k),k=d.children[n],k.level<b?f.push(k):c.push(k)}return c}childIndex(a){var [b,c]=a.map(a=>.5>a?0:1);return b<<1|c}height(a){if(!this.root)return 0;for(var b=null,c=[this.root];;){var f=c[c.length-1],d=f.height(a);null!==d&&(b=d);d=f.uv(a);d=this.childIndex(d);if(f.children[d])c.push(f.children[d]);else break}return b+this.pointcloud.position.z}update(a){null!==this.matrix&&this.matrix.equals(this.pointcloud.matrixWorld)||(this.matrix=
this.pointcloud.matrixWorld.clone(),this.boundingBox=this.pointcloud.boundingBox.clone().applyMatrix4(this.matrix),this.root=new ea('r',this.boundingBox,this.tileSize),this.version++);var b=null,c;for(c of a)if(void 0===c.demVersion||c.demVersion<this.version){b=c;break}if(null!==b){var f=b.getBoundingBox().clone().applyMatrix4(this.matrix),d=f.getSize(new THREE.Vector3),l=this.expandAndFindByBox(f,b.getLevel());b.demVersion=this.version;a=b.geometryNode.geometry.attributes.position.array;b={boundingBox:{min:b.getBoundingBox().min.toArray(),
max:b.getBoundingBox().max.toArray()},position:(new Float32Array(a)).buffer};var k=this;w.workerPool.runTask(A.DEM,function(a){a=new Float32Array(a.data.dem.data);for(var b of l){for(var c=b.box.getSize(new THREE.Vector3),v=0;v<k.tileSize;v++)for(var e=0;e<k.tileSize;e++){var p=k.tileSize*(b.box.min.x+v/(k.tileSize-1)*c.x-f.min.x)/d.x,g=k.tileSize*(b.box.min.y+e/(k.tileSize-1)*c.y-f.min.y)/d.y;0>p||p>k.tileSize||0>g||g>k.tileSize||(p=Math.min(Math.floor(p),k.tileSize-1),g=Math.min(Math.floor(g),k.tileSize-
1),b.data[v+k.tileSize*e]=a[p+k.tileSize*g])}b.createMipMap();b.mipMapNeedsUpdate=!0}},b,[b.position])}}}class U{constructor(){this.needsTransformUpdate=!0}getChildren(){}getBoundingBox(){}isLoaded(){}isGeometryNode(){}isTreeNode(){}getLevel(){}getBoundingSphere(){}}class R extends THREE.Object3D{constructor(){super();this.dem=new qa(this)}initialized(){return null!==this.root}}class ra{constructor(){this.spacing=0;this.nodes=this.root=this.boundingBox=null;this.pointAttributes={};this.hierarchyStepSize=
-1;this.serverURL=this.boundingSphere=this.projection=this.offset=this.baseDepth=this.schema=this.loader=null;this.normalize={color:!1,intensity:!1}}}y.IDCount=0;y.prototype=Object.create(U.prototype);y.prototype.isGeometryNode=function(){return!0};y.prototype.isTreeNode=function(){return!1};y.prototype.isLoaded=function(){return this.loaded};y.prototype.getBoundingSphere=function(){return this.boundingSphere};y.prototype.getBoundingBox=function(){return this.boundingBox};y.prototype.getLevel=function(){return this.level};
y.prototype.getChildren=function(){for(var a=[],b=0;8>b;++b)this.children[b]&&a.push(this.children[b]);return a};y.prototype.getURL=function(){var a=this.greyhoundBounds;a=''+this.pcoGeometry.serverURL+'read?depthBegin='+(this.baseLoaded?this.level+this.pcoGeometry.baseDepth:0)+'&depthEnd='+(this.level+this.pcoGeometry.baseDepth+1)+'&bounds=['+(a.min.x+','+a.min.y+','+a.min.z+','+a.max.x+','+a.max.y+','+a.max.z)+']&schema='+JSON.stringify(this.pcoGeometry.schema)+'&compress=true';this.scale&&(a+=
'&scale='+this.scale);if(this.greyhoundOffset){var b=this.greyhoundOffset;a+='&offset=['+b.x+','+b.y+','+b.z+']'}this.baseLoaded||(this.baseLoaded=!0);return a};y.prototype.addChild=function(a){this.children[a.index]=a;a.parent=this};y.prototype.load=function(){!0===this.loading||!0===this.loaded||w.numNodesLoading>=w.maxNodesLoading||(this.loading=!0,w.numNodesLoading++,0===this.level%this.pcoGeometry.hierarchyStepSize&&this.hasChildren?this.loadHierarchyThenPoints():this.loadPoints())};y.prototype.loadPoints=
function(){this.pcoGeometry.loader.load(this)};y.prototype.loadHierarchyThenPoints=function(){var a=[0,2,1,3,4,6,5,7],b=function(b){var c=0;Object.keys(b).forEach(function(b){'swd'===b?c+=1<<a[0]:'nwd'===b?c+=1<<a[1]:'swu'===b?c+=1<<a[2]:'nwu'===b?c+=1<<a[3]:'sed'===b?c+=1<<a[4]:'ned'===b?c+=1<<a[5]:'seu'===b?c+=1<<a[6]:'neu'===b&&(c+=1<<a[7])});return c},c=function(d,f,l){var k,h;Object.keys(d).forEach(function(v){if('n'!==v){switch(v){case 'swd':k=d.swd;h=f+a[0];break;case 'nwd':k=d.nwd;h=f+a[1];
break;case 'swu':k=d.swu;h=f+a[2];break;case 'nwu':k=d.nwu;h=f+a[3];break;case 'sed':k=d.sed;h=f+a[4];break;case 'ned':k=d.ned;h=f+a[5];break;case 'seu':k=d.seu;h=f+a[6];break;case 'neu':k=d.neu,h=f+a[7]}l.push({children:b(k),numPoints:k.n,name:h});c(k,h,l)}})};if(0===this.level%this.pcoGeometry.hierarchyStepSize){var f=this.level+this.pcoGeometry.baseDepth,d=this.greyhoundBounds,l=''+this.pcoGeometry.serverURL+'hierarchy?bounds=['+(d.min.x+','+d.min.y+','+d.min.z+','+d.max.x+','+d.max.y+','+d.max.z)+
']&depthBegin='+f+'&depthEnd='+(f+this.pcoGeometry.hierarchyStepSize+2);this.scale&&(l+='&scale='+this.scale);this.greyhoundOffset&&(f=this.greyhoundOffset,l+='&offset=['+f.x+','+f.y+','+f.z+']');var k=this,v=new XMLHttpRequest;v.overrideMimeType('text/plain');v.open('GET',l,!0);v.onload=function(a){try{var b=JSON.parse(v.responseText)||{};a=[];k.numPoints=b.n;c(b,k.name,a);b={};b[k.name]=k;for(var d=k.pcoGeometry,f=0;f<a.length;f++){var l=a[f].name,h=a[f].numPoints,e=parseInt(l.charAt(l.length-1)),
g=l.substring(0,l.length-1),x=b[g],t=l.length-1,u=E.createChildAABB(x.boundingBox,e),z=new y(l,d,u,k.scale,k.offset);z.level=t;z.numPoints=h;z.hasChildren=0<a[f].children;z.spacing=d.spacing/Math.pow(2,t);x.addChild(z);b[l]=z}k.loadPoints()}catch(Ia){w.numNodesLoading--,console.error('Potree: Exception thrown parsing points.',Ia)}};v.onerror=function(a){console.log('Potree: Failed to load file! HTTP status '+v.status+', file:'+l,a)};v.send(null)}};y.prototype.getNumPoints=function(){return this.numPoints};
y.prototype.dispose=function(){if(this.geometry&&null!=this.parent){this.geometry.dispose();this.geometry=null;this.loaded=!1;for(var a=0;a<this.oneTimeDisposeHandlers.length;a++)(0,this.oneTimeDisposeHandlers[a])();this.oneTimeDisposeHandlers=[]}};Object.assign(y.prototype,THREE.EventDispatcher.prototype);z.prototype.newerThan=function(a){a=new z(a);return this.versionMajor>a.versionMajor||this.versionMajor===a.versionMajor&&this.versionMinor>a.versionMinor?!0:!1};z.prototype.equalOrHigher=function(a){a=
new z(a);return this.versionMajor>a.versionMajor||this.versionMajor===a.versionMajor&&this.versionMinor>=a.versionMinor?!0:!1};z.prototype.upTo=function(a){return!this.newerThan(a)};class sa{constructor(a,b,c){this.version='string'===typeof a?new z(a):a;this.boundingBox=b;this.scale=c}load(a){if(!a.loaded){var b=this,c=a.getURL(),f=new XMLHttpRequest;f.overrideMimeType('text/plain');f.open('GET',c,!0);f.responseType='arraybuffer';f.overrideMimeType('text/plain; charset=x-user-defined');f.onload=function(){try{b.parse(a,
f.response)}catch(d){console.error('Potree: Exception thrown parsing points.',d),w.numNodesLoading--}};f.onerror=function(a){w.numNodesLoading--;console.error('Potree: Failed to load file.',f,c)};f.send(null)}}parse(a,b){var c=(new DataView(b,b.byteLength-4,4)).getUint32(0,!0),f=a.pcoGeometry.pointAttributes;a.numPoints=c;c=a.boundingBox;var d=new THREE.Vector3;d=a.pcoGeometry.boundingBox.getCenter(d).sub(a.boundingBox.min);b={buffer:b,pointAttributes:f,version:this.version.version,schema:a.pcoGeometry.schema,
min:[c.min.x,c.min.y,c.min.z],max:[c.max.x,c.max.y,c.max.z],offset:d.toArray(),scale:this.scale,normalize:a.pcoGeometry.normalize};w.workerPool.runTask(A.GREYHOUND,function(b){b=b.data;var c=b.attributeBuffers,d=new THREE.Box3((new THREE.Vector3).fromArray(b.tightBoundingBox.min),(new THREE.Vector3).fromArray(b.tightBoundingBox.max)),f=new THREE.BufferGeometry,l;for(l in c){var e=c[l].buffer;parseInt(l)===u.POSITION_CARTESIAN?f.setAttribute('position',new THREE.BufferAttribute(new Float32Array(e),
...
this.numNodes=0;this.root=this.url=this.provider=this.name=null;this.levels=0;this._spacing=null;this.pointAttributes=new t(['POSITION_CARTESIAN','COLOR_PACKED'])}static load(a,b){var c=new XMLHttpRequest;c.overrideMimeType('text/plain');c.open('GET',a+'?info',!0);c.onreadystatechange=function(){try{if(4!==c.readyState||200!==c.status&&0!==c.status)4===c.readyState&&b(null);else{var f=JSON.parse(c.responseText),d=new ha;d.url=a;d.name=f.Name;d.provider=f.Provider;d.numNodes=f.Nodes;d.numPoints=f.Points;
d.version=f.Version;d.boundingBox=new THREE.Box3((new THREE.Vector3).fromArray(f.BoundingBox.slice(0,3)),(new THREE.Vector3).fromArray(f.BoundingBox.slice(3,6)));f.Spacing&&(d.spacing=f.Spacing);var e=d.boundingBox.min.clone().multiplyScalar(-1);d.boundingBox.min.add(e);d.boundingBox.max.add(e);d.offset=e;var k=new THREE.Vector3;d.boundingBox.getCenter(k);var g=d.boundingBox.getSize(new THREE.Vector3).length()/2;d.boundingSphere=new THREE.Sphere(k,g);d.loadHierarchy();b(d)}}catch(h){console.error(h.message),
b(null)}};c.send(null)}loadHierarchy(){var a=this.url+'?tree',b=new XMLHttpRequest;b.overrideMimeType('text/plain');b.open('GET',a,!0);b.responseType='arraybuffer';b.onreadystatechange=()=>{if(4===b.readyState&&(200===b.status||0===b.status)){var a=b.response,f=a.byteLength/3;a=new DataView(a);for(var d=[],e=null,g=0,v=0;v<f;v++){var h=a.getUint8(3*v,!0),m=0<(h&1),n=0<(h&2),q=0<(h&8),p=0<(h&16),r=null;0<(h&4)?r='X':q&&(r='Y');p&&(r='Z');h=new Ma;h.hasLeft=m;h.hasRight=n;h.split=r;h.isLeaf=!m&&!n;
h.number=v;h.left=null;h.right=null;h.pcoGeometry=this;h.level=d.length;g=Math.max(g,h.level);0<d.length?(m=d[d.length-1],h.boundingBox=m.boundingBox.clone(),n=m.boundingBox.getSize(new THREE.Vector3),m.hasLeft&&!m.left?(m.left=h,m.children.push(h),'X'===m.split?h.boundingBox.max.x=h.boundingBox.min.x+n.x/2:'Y'===m.split?h.boundingBox.max.y=h.boundingBox.min.y+n.y/2:'Z'===m.split&&(h.boundingBox.max.z=h.boundingBox.min.z+n.z/2),m=new THREE.Vector3,h.boundingBox.getCenter(m),n=h.boundingBox.getSize(new THREE.Vector3).length()/
2):(m.right=h,m.children.push(h),'X'===m.split?h.boundingBox.min.x+=n.x/2:'Y'===m.split?h.boundingBox.min.y+=n.y/2:'Z'===m.split&&(h.boundingBox.min.z+=n.z/2),m=new THREE.Vector3,h.boundingBox.getCenter(m),n=h.boundingBox.getSize(new THREE.Vector3).length()/2),h.boundingSphere=new THREE.Sphere(m,n)):(e=h,e.boundingBox=this.boundingBox.clone(),m=new THREE.Vector3,e.boundingBox.getCenter(m),n=e.boundingBox.getSize(new THREE.Vector3).length()/2,e.boundingSphere=new THREE.Sphere(m,n));m=h.boundingBox.getSize(new THREE.Vector3);
h.spacing=(m.x+m.y+m.z)/3/75;h.estimatedSpacing=h.spacing;d.push(h);if(h.isLeaf)for(h=!1;!h&&0<d.length;)d.pop(),h=d[d.length-1],h=0<d.length&&h.hasRight&&null==h.right}this.root=e;this.levels=g;this.dispatchEvent({type:'hierarchy_loaded'})}};b.send(null)}get spacing(){if(this._spacing)return this._spacing;if(this.root)return this.root.spacing}set spacing(a){this._spacing=a}}da.prototype={push:function(a){this.content.push(a);this.bubbleUp(this.content.length-1)},pop:function(){var a=this.content[0],
...
d;a=f}},sinkDown:function(a){for(var b=this.content.length,c=this.content[a],f=this.scoreFunction(c);;){var d=2*(a+1),e=d-1,g=null;if(e<b){var v=this.scoreFunction(this.content[e]);v<f&&(g=e)}d<b&&this.scoreFunction(this.content[d])<(null==g?f:v)&&(g=d);if(null==g)break;this.content[a]=this.content[g];this.content[g]=c;a=g}}};var X={position:0,color:1,intensity:2,classification:3,returnNumber:4,numberOfReturns:5,pointSourceID:6,indices:7,normal:8,spacing:9},Ca={DEFAULT:{0:new THREE.Vector4(.5,.5,
.5,1),1:new THREE.Vector4(.5,.5,.5,1),2:new THREE.Vector4(.63,.32,.18,1),3:new THREE.Vector4(0,1,0,1),4:new THREE.Vector4(0,.8,0,1),5:new THREE.Vector4(0,.6,0,1),6:new THREE.Vector4(1,.66,0,1),7:new THREE.Vector4(1,0,1,1),8:new THREE.Vector4(1,0,0,1),9:new THREE.Vector4(0,0,1,1),12:new THREE.Vector4(1,1,0,1),DEFAULT:new THREE.Vector4(.3,.6,.6,.5)}},S={NONE:0,HIGHLIGHT:1,SHOW_INSIDE:2,SHOW_OUTSIDE:3},P={FIXED:0,ATTENUATED:1,ADAPTIVE:2},W={SQUARE:0,CIRCLE:1,PARABOLOID:2},B={RGB:0,COLOR:1,DEPTH:2,HEIGHT:3,
ELEVATION:3,INTENSITY:4,INTENSITY_GRADIENT:5,LOD:6,LEVEL_OF_DETAIL:6,POINT_INDEX:7,CLASSIFICATION:8,RETURN_NUMBER:9,SOURCE:10,NORMAL:11,PHONG:12,RGB_HEIGHT:13,COMPOSITE:50},V={OCTREE:0,KDTREE:1};class Oa{constructor(){this.boundingBox=new THREE.Box3;this.numPoints=0;this.data={}}add(a){var b=this.numPoints,c=b+a.numPoints,f=Object.keys(this.data),d=Object.keys(a.data),e=new Set([...f,...d]),g;for(g of e)if(f.includes(g)&&d.includes(g)){var v=this.data[g].constructor;e=new v(this.data[g].length+a.data[g].length);
e.set(this.data[g],0);e.set(a.data[g],this.data[g].length);this.data[g]=e}else f.includes(g)&&!d.includes(g)?(e=this.data[g].length/this.numPoints,v=this.data[g].constructor,v=new v(e*c),v.set(this.data[g],0),this.data[g]=v):!f.includes(g)&&d.includes(g)&&(e=a.data[g].length/a.numPoints,v=a.data[g].constructor,v=new v(e*c),v.set(a.data[g],e*b),this.data[g]=v);this.numPoints=c;this.boundingBox.union(a.boundingBox)}}class ia{constructor(a,b){this.gl=a;this.texture=b;this.id=a.createTexture();this.target=
...
b){var c=new Fa;c.vao=a.createVertexArray();c.numElements=b.attributes.position.count;a.bindVertexArray(c.vao);for(var e in b.attributes){var d=b.attributes[e],g=a.createBuffer();a.bindBuffer(a.ARRAY_BUFFER,g);a.bufferData(a.ARRAY_BUFFER,d.array,a.STATIC_DRAW);var k=X[e],v=d.normalized,h=this.types.get(d.array.constructor);void 0!==h&&(a.vertexAttribPointer(k,d.itemSize,h,v,0,0),a.enableVertexAttribArray(k));c.vbos.set(e,{handle:g,name:e,count:d.count,itemSize:d.itemSize,type:b.attributes.position.array.constructor,
version:0})}a.bindBuffer(a.ARRAY_BUFFER,null);a.bindVertexArray(null);return c}updateBuffer(a,b){var c=this.buffers.get(b);a.bindVertexArray(c.vao);for(var e in b.attributes){var d=b.attributes[e],g=X[e],k=d.normalized,v=this.types.get(d.array.constructor);if(c.vbos.has(e)){var h=c.vbos.get(e).handle;c.vbos.get(e).version=d.version}else h=a.createBuffer(),c.vbos.set(e,{handle:h,name:e,count:d.count,itemSize:d.itemSize,type:b.attributes.position.array.constructor,version:d.version});a.bindBuffer(a.ARRAY_BUFFER,
h);a.bufferData(a.ARRAY_BUFFER,d.array,a.STATIC_DRAW);a.vertexAttribPointer(g,d.itemSize,v,k,0,0);a.enableVertexAttribArray(g)}a.bindBuffer(a.ARRAY_BUFFER,null);a.bindVertexArray(null)}fetchOctrees(){for(var a=[],b=[this];0<b.length;){var c=b.pop();c instanceof R?a.push(c):(c=c.children.filter(a=>a.visible),b.push(...c))}return{octrees:a}}renderNodes(a,b,c,e,d,g){a=a.getContext();b=b.material;d=d.matrixWorldInverse;var f=new THREE.Matrix4,l=new Float32Array(16),h;for(h of c)if(void 0===w.debug.allowedNodes||
w.debug.allowedNodes.includes(h.name)){var m=h.sceneNode.matrixWorld;f.multiplyMatrices(d,m);e&&(c=e.offsets.get(h),g.setUniform1f('uVNStart',c));c=h.getLevel();g.setUniform('uDebug',!0===h.debug);if(h instanceof aa)var n=0===Object.keys(h.children).length;else h instanceof fa&&(n=h.geometryNode.isLeaf);g.setUniform('uIsLeafNode',n);var q=g.uniformLocations.modelMatrix;q&&(l.set(m.elements),a.uniformMatrix4fv(q,!1,l));q=g.uniformLocations.modelViewMatrix;l.set(f.elements);a.uniformMatrix4fv(q,!1,
l);if(b.clipPolygons&&0<b.clipPolygons.length){q=[];var p=[];for(t of b.clipPolygons){d=t.viewMatrix;var r=t.projMatrix.clone().multiply(d).multiply(m);q.push(t.markers.length);p.push(r)}m=[].concat(...p.map(a=>a.elements));r=Array(24*b.clipPolygons.length);for(p=0;p<b.clipPolygons.length;p++){var t=b.clipPolygons[p];for(var u=0;u<t.markers.length;u++)r[24*p+3*u]=t.markers[u].position.x,r[24*p+(3*u+1)]=t.markers[u].position.y,r[24*p+(3*u+2)]=t.markers[u].position.z}a.uniform1iv(g.uniformLocations['uClipPolygonVCount[0]'],
q);a.uniformMatrix4fv(g.uniformLocations['uClipPolygonWVP[0]'],!1,m);a.uniform3fv(g.uniformLocations['uClipPolygonVertices[0]'],r)}g.setUniform1f('uLevel',c);g.setUniform1f('uNodeSpacing',h.geometryNode.estimatedSpacing);g.setUniform1f('uPCIndex',p);c=h.geometryNode.geometry;m=null;if(this.buffers.has(c)){m=this.buffers.get(c);for(var x in c.attributes)c.attributes[x].version>m.vbos.get(x).version&&this.updateBuffer(a,c)}else m=this.createBuffer(a,c),this.buffers.set(c,m);a.bindVertexArray(m.vao);
a.drawArrays(a.POINTS,0,m.numElements)}a.bindVertexArray(null)}renderOctree(a,b,c,e){var d=a.getContext(),f=b.material,g=e.matrixWorldInverse,t=e.matrixWorld,h=e.projectionMatrix,m=(new THREE.Matrix4).getInverse(h);new THREE.Matrix4;var n=null,q=0;if(f.pointSizeType===P.ADAPTIVE||f.pointColorType===B.LOD){n=b.computeVisibilityTextureData(c,e);var p=f.visibleNodesTexture;p.image.data.set(n.data);p.needsUpdate=!0}p=null;this.shaders.has(f)?p=this.shaders.get(f):(p=new Ea(d,'pointcloud',f.vertexShader,
f.fragmentShader),this.shaders.set(f,p));var r=['#define num_shadowmaps0','#define num_snapshots'+(f.snapEnabled?f.numSnapshots:0),'#define num_clipboxes'+(f.clipBoxes&&f.clipBoxes.length?f.clipBoxes.length:0),'#define num_clipspheres0','#define num_clippolygons'+(f.clipPolygons&&f.clipPolygons.length?f.clipPolygons.length:0)].join('\n');p.update(r+'\n'+f.vertexShader,r+'\n'+f.fragmentShader);f.needsUpdate=!1;for(var u of Object.keys(f.uniforms))if(r=f.uniforms[u],'t'==r.type&&(r=r.value)){if(!this.textures.has(r)){var w=
new ia(d,r);this.textures.set(r,w)}this.textures.get(r).update()}d.useProgram(p.program);1>f.opacity?(d.enable(d.BLEND),d.blendFunc(d.SRC_ALPHA,d.ONE),d.depthMask(!1),d.disable(d.DEPTH_TEST)):(d.disable(d.BLEND),d.depthMask(!0),d.enable(d.DEPTH_TEST));p.setUniformMatrix4('projectionMatrix',h);p.setUniformMatrix4('viewMatrix',g);p.setUniformMatrix4('uViewInv',t);p.setUniformMatrix4('uProjInv',m);r=f.screenHeight;p.setUniform1f('uScreenWidth',f.screenWidth);p.setUniform1f('uScreenHeight',r);p.setUniform1f('fov',
Math.PI*e.fov/180);p.setUniform1f('near',e.near);p.setUniform1f('far',e.far);a.capabilities.logarithmicDepthBuffer&&p.setUniform('logDepthBufFC',2/(Math.log(e.far+1)/Math.LN2));e instanceof THREE.OrthographicCamera?(p.setUniform('uUseOrthographicCamera',!0),p.setUniform('uOrthoWidth',e.right-e.left),p.setUniform('uOrthoHeight',e.top-e.bottom)):p.setUniform('uUseOrthographicCamera',!1);0===f.clipBoxes.length+f.clipPolygons.length?p.setUniform1i('clipTask',S.NONE):p.setUniform1i('clipTask',f.clipTask);
p.setUniform1i('clipMethod',f.clipMethod);f.clipBoxes&&0<f.clipBoxes.length&&d.uniformMatrix4fv(p.uniformLocations['clipBoxes[0]'],!1,f.uniforms.clipBoxes.value);p.setUniform1f('size',f.size);p.setUniform1f('maxSize',f.uniforms.maxSize.value);p.setUniform1f('minSize',f.uniforms.minSize.value);p.setUniform1f('uOctreeSpacing',f.spacing);p.setUniform('uOctreeSize',f.uniforms.octreeSize.value);p.setUniform3f('uColor',f.color.toArray());p.setUniform1f('uOpacity',f.opacity);p.setUniform2f('elevationRange',
f.elevationRange);p.setUniform2f('intensityRange',f.intensityRange);p.setUniform1f('intensityGamma',f.intensityGamma);p.setUniform1f('intensityContrast',f.intensityContrast);p.setUniform1f('intensityBrightness',f.intensityBrightness);p.setUniform1f('rgbGamma',f.rgbGamma);p.setUniform1f('rgbContrast',f.rgbContrast);p.setUniform1f('rgbBrightness',f.rgbBrightness);p.setUniform1f('uTransition',f.transition);p.setUniform1f('wRGB',f.weightRGB);p.setUniform1f('wIntensity',f.weightIntensity);p.setUniform1f('wElevation',
f.weightElevation);p.setUniform1f('wClassification',f.weightClassification);p.setUniform1f('wReturnNumber',f.weightReturnNumber);p.setUniform1f('wSourceID',f.weightSourceID);r=this.textures.get(f.visibleNodesTexture);p.setUniform1i('visibleNodesTexture',q);d.activeTexture(d.TEXTURE0+q);d.bindTexture(r.target,r.id);q++;r=this.textures.get(f.gradientTexture);p.setUniform1i('gradient',q);d.activeTexture(d.TEXTURE0+q);d.bindTexture(r.target,r.id);q++;r=this.textures.get(f.classificationTexture);p.setUniform1i('classificationLUT',
q);d.activeTexture(d.TEXTURE0+q);d.bindTexture(r.target,r.id);q++;if(!0===f.snapEnabled){r=p.uniformLocations['uSnapshot[0]'];h=p.uniformLocations['uSnapshotDepth[0]'];g=Array(5).fill(q).map((a,b)=>a+b);t=Array(5).fill(1+Math.max(...g)).map((a,b)=>a+b);q=1+Math.max(...t);d.uniform1iv(r,g);d.uniform1iv(h,t);for(q=0;5>q;q++){r=f.uniforms.uSnapshot.value[q];h=f.uniforms.uSnapshotDepth.value[q];if(!r)break;r=a.properties.get(r).__webglTexture;h=a.properties.get(h).__webglTexture;m=t[q];d.activeTexture(d[`TEXTURE${g[q]}`]);
d.bindTexture(d.TEXTURE_2D,r);d.activeTexture(d[`TEXTURE${m}`]);d.bindTexture(d.TEXTURE_2D,h)}q=[].concat(...f.uniforms.uSnapView.value.map(a=>a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapView[0]'],!1,q);q=[].concat(...f.uniforms.uSnapProj.value.map(a=>a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapProj[0]'],!1,q);q=[].concat(...f.uniforms.uSnapProjInv.value.map(a=>a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapProjInv[0]'],!1,q);q=[].concat(...f.uniforms.uSnapViewInv.value.map(a=>
a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapViewInv[0]'],!1,q)}this.renderNodes(a,b,c,n,e,p);d.activeTexture(d.TEXTURE2);d.bindTexture(d.TEXTURE_2D,null);d.activeTexture(d.TEXTURE0)}}e.AttributeLocations=X;e.BasicGroup=Ga;e.BinaryHeap=da;e.BinaryLoader=ta;e.Classification=Ca;e.ClipMethod={INSIDE_ANY:0,INSIDE_ALL:1};e.ClipTask=S;e.DEM=qa;e.DEMNode=ea;e.EptBinaryLoader=wa;e.EptLaszipLoader=xa;e.EptLoader=Aa;e.Global=w;e.Gradients=Ba;e.GreyhoundBinaryLoader=sa;e.GreyhoundLoader=E;e.GreyhoundUtils=
H;e.Group=Pa;e.HelperUtils=L;e.LASLAZLoader=ua;e.LASLoader=Q;e.LRU=na;e.POCLoader=Y;e.PointAttribute=g;e.PointAttributeNames=u;e.PointAttributeTypes=C;e.PointAttributes=t;e.PointCloudArena4D=Da;e.PointCloudArena4DGeometry=ha;e.PointCloudEptGeometry=ya;e.PointCloudGreyhoundGeometry=ra;e.PointCloudMaterial=M;e.PointCloudOctree=ba;e.PointCloudOctreeGeometry=va;e.PointCloudTree=R;e.PointColorType=B;e.PointShape=W;e.PointSizeType=P;e.Points=Oa;e.Shader=Ea;e.Shaders=O;e.TreeType=V;e.VersionUtils=z;e.WebGLBuffer=
Fa;e.WebGLTexture=ia;e.WorkerManager=A;e.loadPointCloud=function(a,b,c){var e=function(a){void 0!==b&&(a.name=b);c({type:'pointcloud_loaded',pointcloud:a})};if(0===a.indexOf('greyhound://'))E.load(a,function(a){void 0!==a&&e(new ba(a))});else if(0<a.indexOf('cloud.js'))Y.load(a,function(a){void 0!==a&&e(new ba(a))});else if(0<a.indexOf('ept.json'))Aa.load(a,function(a){void 0!==a&&e(new ba(a))});else if(0<a.indexOf('.vpc'))ha.load(a,function(a){void 0!==a&&e(new Da(a))});else throw Error('Potree: Failed to load point cloud from URL '+
a);};e.updatePointClouds=ma;e.updateVisibility=ka;e.updateVisibilityStructures=la;Object.defineProperty(e,'__esModule',{value:!0})});
