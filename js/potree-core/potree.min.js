...
(b+='.bin');var c=this,f=new XMLHttpRequest;f.open('GET',b,!0);f.responseType='arraybuffer';f.overrideMimeType('text/plain; charset=x-user-defined');f.onload=function(){try{c.parse(a,f.response)}catch(d){w.numNodesLoading--,console.error('Potree: Exception thrown parsing points.',d)}};f.onerror=function(a){w.numNodesLoading--;console.error('Potree: Failed to load file.',f,b)};f.send(null);console.log('Loading '+b)}}parse(a,b){var c=a.pcoGeometry.pointAttributes,f=b.byteLength/a.pcoGeometry.pointAttributes.byteSize;
this.version.upTo('1.5')&&(a.numPoints=f);b={buffer:b,pointAttributes:c,version:this.version.version,min:[a.boundingBox.min.x,a.boundingBox.min.y,a.boundingBox.min.z],offset:[a.pcoGeometry.offset.x,a.pcoGeometry.offset.y,a.pcoGeometry.offset.z],scale:this.scale,spacing:a.spacing,hasChildren:a.hasChildren,name:a.name};w.workerPool.runTask(A.BINARY_DECODER,function(b){var d=b.data;if(void 0!==d.error)w.numNodesLoading--,console.error('Potree: Binary worker error.',d);else{var f=d.attributeBuffers,e=
new THREE.Box3((new THREE.Vector3).fromArray(d.tightBoundingBox.min),(new THREE.Vector3).fromArray(d.tightBoundingBox.max)),h=new THREE.BufferGeometry,m;for(m in f){var n=f[m].buffer;parseInt(m)===u.POSITION_CARTESIAN?h.setAttribute('position',new THREE.BufferAttribute(new Float32Array(n),3)):parseInt(m)===u.COLOR_PACKED?h.setAttribute('color',new THREE.BufferAttribute(new Uint8Array(n),4,!0)):parseInt(m)===u.INTENSITY?h.setAttribute('intensity',new THREE.BufferAttribute(new Float32Array(n),1)):parseInt(m)===
u.CLASSIFICATION?h.setAttribute('classification',new THREE.BufferAttribute(new Uint8Array(n),1)):parseInt(m)===u.NORMAL_SPHEREMAPPED?h.setAttribute('normal',new THREE.BufferAttribute(new Float32Array(n),3)):parseInt(m)===u.NORMAL_OCT16?h.setAttribute('normal',new THREE.BufferAttribute(new Float32Array(n),3)):parseInt(m)===u.NORMAL?h.setAttribute('normal',new THREE.BufferAttribute(new Float32Array(n),3)):parseInt(m)===u.INDICES?(n=new THREE.BufferAttribute(new Uint8Array(n),4),n.normalized=!0,h.setAttribute('indices',
n)):parseInt(m)===u.SPACING&&(n=new THREE.BufferAttribute(new Float32Array(n),1),h.setAttribute('spacing',n))}e.max.sub(e.min);e.min.set(0,0,0);a.numPoints=b.data.buffer.byteLength/c.byteSize;a.geometry=h;a.mean=new THREE.Vector3(...d.mean);a.tightBoundingBox=e;a.loaded=!0;a.loading=!1;a.estimatedSpacing=d.estimatedSpacing;w.numNodesLoading--}},b,[b.buffer])}}var ja=[function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,
!0)}},function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0)}},function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0),color:[a.getUint16(20,!0),a.getUint16(22,!0),a.getUint16(24,!0)]}},function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0),
color:[a.getUint16(28,!0),a.getUint16(30,!0),a.getUint16(32,!0)]}}];Q.prototype.open=function(){this.readOffset=0;return new Promise(function(a,b){setTimeout(a,0)})};Q.prototype.getHeader=function(){var a=this;return new Promise(function(b,c){setTimeout(function(){var c=a.arraybuffer,d={};d.pointsOffset=I(c,Uint32Array,96);d.pointsFormatId=I(c,Uint8Array,104);d.pointsStructSize=I(c,Uint16Array,105);d.pointsCount=I(c,Uint32Array,107);var l=131;d.scale=I(c,Float64Array,l,3);l+=24;d.offset=I(c,Float64Array,
l,3);c=I(c,Float64Array,l+24,6);d.maxs=[c[0],c[2],c[4]];d.mins=[c[1],c[3],c[5]];a.header=d;b(a.header)},0)})};Q.prototype.readData=function(a,b,c){var f=this;return new Promise(function(b,l){setTimeout(function(){if(!f.header)return l(Error('Cannot start reading data till a header request is issued'));if(1>=c){a=Math.min(a,f.header.pointsCount-f.readOffset);var d=f.header.pointsOffset+f.readOffset*f.header.pointsStructSize;b({buffer:f.arraybuffer.slice(d,d+a*f.header.pointsStructSize),count:a,hasMoreData:f.readOffset+
a<f.header.pointsCount});f.readOffset+=a}else{for(var e=Math.min(a*c,f.header.pointsCount-f.readOffset),h=0,m=new Uint8Array(Math.ceil(e/c)*f.header.pointsStructSize),n=0;n<e;n++)0===n%c&&(d=f.header.pointsOffset+f.readOffset*f.header.pointsStructSize,d=new Uint8Array(f.arraybuffer,d,f.header.pointsStructSize),m.set(d,h*f.header.pointsStructSize),h++),f.readOffset++;b({buffer:m.buffer,count:h,hasMoreData:f.readOffset<f.header.pointsCount})}},0)})};Q.prototype.close=function(){var a=this;return new Promise(function(b,
c){a.arraybuffer=null;setTimeout(b,0)})};T.prototype.open=function(){var a=this;return new Promise(function(b,c){a.dorr({type:'open',arraybuffer:a.arraybuffer},function(a){if(1!==a.status)return c(Error('Failed to open file'));b(!0)})})};T.prototype.getHeader=function(){var a=this;return new Promise(function(b,c){a.dorr({type:'header'},function(a){if(1!==a.status)return c(Error('Failed to get header'));b(a.header)})})};T.prototype.readData=function(a,b,c){var f=this;return new Promise(function(d,
l){f.dorr({type:'read',count:a,offset:b,skip:c},function(a){if(1!==a.status)return l(Error('Failed to read data'));d({buffer:a.buffer,count:a.count,hasMoreData:a.hasMoreData})})})};T.prototype.close=function(){var a=this;return new Promise(function(b,c){a.dorr({type:'close'},function(a){if(1!==a.status)return c(Error('Failed to close file'));b(!0)})})};J.prototype.determineFormat=function(){var a=I(this.arraybuffer,Uint8Array,104),b=(a&128)>>7,c=(a&64)>>6;if(1===b&&1===c)throw Error('Old style compression not supported');
...
c.overrideMimeType('text/plain; charset=x-user-defined');c.onreadystatechange=()=>{4===c.readyState&&(200===c.status?this.parse(a,c.response):console.log('Failed '+b+': '+c.status))};c.send(null)}}parse(a,b){var c=new J(b),f=new La(a);c.open().then(()=>{c.isOpen=!0;return c.getHeader()}).then(b=>{var d=0,k=a=>[a.x,a.y,a.z],e=k(a.key.b.min),h=k(a.key.b.max),m=()=>c.readData(1E6,0,1).then(function(a){var l=new ca(a.buffer,b.pointsFormatId,b.pointsStructSize,a.count,b.scale,b.offset,e,h);l.extraBytes=
b.extraBytes;l.pointsFormatId=b.pointsFormatId;f.push(l);d+=a.count;if(a.hasMoreData)return m();b.totalRead=d;b.versionAsString=c.versionAsString;b.isCompressed=c.isCompressed;return null});return m()}).then(()=>c.close()).then(()=>c.isOpen=!1).catch(a=>{console.log('Error reading LAZ:',a);if(c.isOpen)c.close().then(()=>{c.isOpen=!1;throw a;});else throw a;})}}class La{constructor(a){this.node=a}push(a){var b=w.workerPool.getWorker(A.EPT_LAS_ZIP_DECODER);b.onmessage=c=>{var d=new THREE.BufferGeometry,
