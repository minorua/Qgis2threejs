...
d.init = (setupDefaultItems = true) => {
	...
	if (setupDefaultItems) {
		...
		if (conf.gui.customPlane) {
			d.customPlaneFolder = panel.addFolder('Custom Plane');
		}
...
d.initLayersFolder = (scene) => {
	...
	scene.forEachLayer((layer, layerId) => {
		...
		folder.add(params.lyr[layerId], 'v').name('Visible').onChange((value) => {
			layer.visible = value;
		});
...
d.initCustomPlaneFolder = (zMin, zMax) => {
	...
	d.customPlaneFolder.addColor(params.cp, 'c').name('Color').onChange((value) => {
		if (d.customPlane === undefined) {
			addPlane(params.cp.c);
		}
	...
	d.customPlaneFolder.add(params.cp, 'l').name('Enlarge').onChange((value) => {
		...
		if (value) {
			d.customPlane.scale.set(80, 80, 1);
		} else {
			d.customPlane.scale.set(1, 1, 1);
		}
		d.customPlane.updateMatrixWorld();
		app.render();
	});
