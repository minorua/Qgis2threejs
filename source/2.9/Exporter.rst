...
* Use original DEM values
   ...
   Uses the original DEM elevation values without interpolation.
...
.. note::
  We are currently revising the layout of the vector layer properties dialog.
  The following explanation is based on the layout of the upcoming version 2.9.3.
...
and the duration of the transition. If text or HTML is entered in the Narrative text box, a popup will be displayed at the time of the
corresponding keyframe during animation playback.
...
* Output directory and HTML filename
   ...
   Geometry data, texture images, and other resource files are exported to a subdirectory with the same name
   as the HTML file, located under the ``data`` directory inside the output directory. Required JavaScript library
...
* Use current view as initial view
   ...
   If enabled, the current view shown in the preview window at the time of export is saved and used as the initial view
